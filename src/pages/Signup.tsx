...
const Signup: React.FC = () => {
    ...
    const { control, register, handleSubmit: handleRegisterSubmit, getValues: getFormValues, reset, formState, trigger: triggerFormValidation } =
        useForm({
            mode: "onChange",
            defaultValues: {
                businessName: "",
    ...
    const [disableGstUsernameProceedBtn] =
        useState(false);
    ...
    const [disableEmailVerifyWithOtpBtn, setDisableEmailVerifyWithOtpBtn] =
        useState(true);
    const [disableEmailVerifyOtpBtn] =
        useState(false);
    ...
    const fetchData = async () => {
        if (inviteId) {
            let apiRes = await signUpService.retrieveDataFromInvite({ businessPartnerInviteId: inviteId });
            let res = apiRes.data
    ...
    async function sendUserMobileOtp() {
        ...
        const res = await signUpService.requestPhoneVerification(
            sendOtpPayload
        );
    ...
    async function sendUserEmailOtp() {
        ...
        const res = await signUpService.requestEmailVerification(
            sendOtpPayload
        );
    ...
    const otherValidations = (canShowToast = true) => {
        ...
        if (canShowToast && !isValid && message)
            toast(message)
        return isValid
    ...
    const basicSignupValidation = async (triggerErrors: boolean) => {
        ...
        let isFormValid = triggerErrors ? await triggerFormValidation() : formState.isValid
        let valid = false
        if (isFormValid) {
    ...
    const FormTemplate = (props: any) => {
        const { handleSubmit, getState, change, initialize, resumeValidation, getFieldState, focus, blur } = useFormApi();
        // @ts-ignore
                ...
                return {
                    ...
                    props: {
                        ...
                        onChange: (value: string) => {
                            ...
                            const formattedInitialState = _.omitBy(initialStateValues.formState, _.isEmpty);
                            const currentFormState = _.omitBy(getState().values, _.isEmpty);

                            setAdditionalFormState({ ...formattedInitialState, ...currentFormState, [field.key]: value })
                        },
                    },
                };
            return field;
        ...
        return (
            <Form
                ...
                onSubmitCapture={(event) => {
                    let formattedInitialState = _.omitBy(initialStateValues.formState, _.isEmpty);
                    let currentFormState = _.omitBy(getState().values, _.isEmpty);
                    event.preventDefault();
                    handleSubmit();
                    const updatedStateValue = { ...formattedInitialState, ...currentFormState }
                ...
                onChange={(e) => {
                    ...
                    const formattedInitialState = _.omitBy(initialStateValues.formState, _.isEmpty);
                    const currentFormState = _.omitBy(getState().values, _.isEmpty);
                    let additionalObj = { ...additionalFormState, ...formattedInitialState, ...currentFormState, [name]: value }
    ...
    return (
        <IonPage className="login-page signup-page pt-11">
            ...
            <IonContent className="white ion-padding">
                ...
                <Tabs className="custom-tabs" selectedIndex={selectedTab} onSelect={(index) => setSelectedTab(index)} >
                    ...
                    <TabPanel>
                        <div className="tab1">
                            <section className="signup-form">
                                <form onSubmit={isAdditionalFormAvailable() ? handleRegisterSubmit(basicSignup) : handleRegisterSubmit(signup)}>
                                    <IonGrid className="pb-0">
                                            ...
                                            <IonCol>
                                                <IonList className="py-0">
                                                    ...
                                                    >
                                                        ...
                                                        {formState.errors?.businessName && <IonText color="danger">{formState.errors?.businessName?.message}</IonText>}
                                                    </IonItem>
                                                </IonList>
                                            </IonCol>
                                        </IonRow>
                                                    ...
                                                    <IonList className="py-0">
                                                        ...
                                                        >
                                                            <IonLabel
                                                                ...
                                                                position="stacked"
                                                            >
                                                                {t("GSTIN_HEADING")} {_.includes(["LVL_1", "LVL_2"], gstRules?.BP_VALIDATION) ? "*" : ""}
                                                            </IonLabel>
                                                            <IonItem
                                                                lines="none"
                                                                className="input-item"
                                                            ...
                                                            {formState.errors?.gstin && <IonText color="danger">{formState.errors?.gstin?.message}</IonText>}
                                                        </IonItem>
                                                    </IonList>
                                                </IonCol>
                                            )}
                                                ...
                                                <IonCol size="7">
                                                    <IonList className="py-0">
                                                        ...
                                                        >
                                                            ...
                                                            {formState.errors?.gstUsername && <IonText color="danger">{formState.errors?.gstUsername?.message}</IonText>}
                                                        </IonItem>
                                                    </IonList>
                                                </IonCol>
                                                <IonCol size="5">
                                                ...
                                                <IonCol size="7">
                                                    <IonList className="py-0">
                                                        ...
                                                        >
                                                            ...
                                                            {formState.errors?.gstOtp && <IonText color="danger">{formState.errors?.gstOtp?.message}</IonText>}
                                                        </IonItem>
                                                    </IonList>
                                                </IonCol>
                                                <IonCol size="5">
                                            ...
                                            <IonCol>
                                                <IonList className="py-0">
                                                    ...
                                                    >
                                                        ...
                                                        {formState.errors?.firstName && <IonText color="danger">{formState.errors?.firstName?.message}</IonText>}
                                                    </IonItem>
                                                </IonList>
                                            </IonCol>
                                        </IonRow>
                                            ...
                                            <IonCol>
                                                <IonList className="py-0">
                                                    ...
                                                    >
                                                        ...
                                                        {formState.errors?.lastName && <IonText color="danger">{formState.errors?.lastName?.message}</IonText>}
                                                    </IonItem>
                                                </IonList>
                                            </IonCol>
                                        </IonRow>
                                            ...
                                            <IonCol size="7">
                                                <IonList className="py-0">
                                                    ...
                                                    >
                                                        ...
                                                        {formState.errors?.applicantMobile && <IonText color="danger">{formState.errors?.applicantMobile?.message}</IonText>}
                                                    </IonItem>
                                                </IonList>
                                            </IonCol>
                                            <IonCol size="5">
                                            ...
                                            <IonCol size="7">
                                                <IonList className="py-0">
                                                    ...
                                                    >
                                                        ...
                                                        {formState.errors?.applicantEmail && <IonText color="danger">{formState.errors?.applicantEmail?.message}</IonText>}
                                                    </IonItem>
                                                </IonList>
                                            </IonCol>
                                            <IonCol size="5">
                                                <IonRow className="ion-justify-content-between">
                                                    ...
                                                    <IonImg onClick={openEmialPopover}
                                                        className=""
                                                        src="./img/i-sign.svg"
                                                    ></IonImg>
                                                    <IonPopover ref={popover} isOpen={emailPopoverOpen} onDidDismiss={() => setEmailPopoverOpen(false)}>
                                                        <IonContent
                                                            class="ion-padding">
                                        ...
                                        <IonRow>
                                            ...
                                            <IonPopover ref={popover} isOpen={passPopoverOpen} onDidDismiss={() => setPassPopoverOpen(false)}>
                                                <IonContent
                                                    class="ion-padding"
                                                    dangerouslySetInnerHTML={{ __html: Constants.PASSWORD_POPOVER_TEXT }}>
                                                </IonContent>
                                            </IonPopover>
                                        </IonRow>
                                            ...
                                            <IonCol size="auto">
                                                ...
                                                checked={isConsentAccepted}                                           
                                                    ...
                                                    <p className="fs-11 fw-100">
                                                        {t("ACCEPT_POLICY")} <a className='footer-links' href={process.env.REACT_APP_TERMS} >
                                                            <span className="primary-color">{t("TERMS")}</span></a>{" "},{" "}
                                                        <a href={process.env.REACT_APP_PRIVACY_POLICY}  >
                                                            <span className="primary-color">{t("POLICY")}</span></a> {" "}
                                                        {t("I_PROVIDE")}{" "}
                                                        <a href={process.env.REACT_APP_CONSENT} >
                                                            <span className="primary-color">{t("MY_CONSENT")}</span></a>{" "}
                                                        {t("CREDIT_INFO")}{" "}<a href={process.env.REACT_APP_TERMS_AND_CONDITIONS}  >
                                                            <span className="primary-color">{t("CONDITIONS")}</span></a> {" "}
                                                        {t("SERVICES")}{" "}
                                                    </p>
                                                </IonItem>

                                                {showConsentErr && <IonText color="danger">{t("FIELD_REQUIRED")}</IonText>}
                                            </IonCol>
                                        </IonRow>
                                        <IonRow>
                                            <IonCol>
                                                ...
                                                >
                                                    ...
                                                    >
                                                        {
                                                            isAdditionalFormAvailable() ? t("NEXT") : 'Submit'
                                                        }
                                                    </IonButton>
                                                </IonRouterLink>
                                            </IonCol>
