...
const GSTReturnPDFUpload: React.FC = () => {
  ...
  const setUploadedCount = (data: IPlatformConfigResponse) => {
    ...
    let uploaded = data?.config?.documents?.GSTR_FILES?.status?.filter(
      (obj) => {
        if (obj.name === "UPLOAD" && obj.value === "COMPLETED") return obj;
        else if (obj.name === "E_VERIFY" && ["COMPLETED"].includes(obj.value))
          return obj;
        return false;
      }
    );
