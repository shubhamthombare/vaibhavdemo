...
import { IBusinessGstinDocumentHistory, IDocument, IFile, IUploadAndSaveDocument } from "../utils/interfaces/DocumentVault.interface";
import { IGstinVerificationList } from "../utils/interfaces/GSTCertificate.interface";
import { getDocumentHistory, uploadAndSaveBusinessDocument, uploadDoc } from "../utils/services/DocumentVault.Service";
...
const GstVerification: React.FC = () => {
  ...
  const [allDocuments, setAllDocuments] = useState<IBusinessGstinDocumentHistory[]>([]);  
  ...
  useEffect(() => {
    ...
    if (selectedGst === '') {
      ...
      if (_.isEmpty(activeDoc.documentInfo))
        setAction('create')
      else
        setAction('details')
  ...
  const submitDocument = async () => {
    ...
    const uploadAndSaveDocumentRequest = generateRequest(uploadObj?.file?.fileId, selectedGst, currentBusinessPartnerId);

    const apiRes = await uploadAndSaveBusinessDocument(uploadAndSaveDocumentRequest);
  ...
  const fetchData = async () => {
    setIsLoading(true)
    let docApiRes = await getDocumentHistory(Constants.DOC_TYPE_GST, currentBusinessPartnerId);
    ...
    if (statusCode === Constants.API_SUCCESS) {
      ...
      if (!selectedGst) {
        if (_.size(dropDownList))
          setSelectedGst(dropDownList[0].value)
      }
  ...
  const isFileSizeValid = (size) => Helpers.isUploadSizeValid(size, Constants.GST_CERTIFICATE_MAX_FILE_SIZE_MB, t("MAXIMUM_FILE_UPLOAD_SIZE"))
  ...
  const getDisplayFileDetails = () => {
    ...
    let statusObj = Helpers.getDocumentStatusForDetails(activeDocument?.eVerificationRequired, activeDocument?.status)
    return {
  ...
  const beginUpload = async (file: any, mime: string) => {
    ...
    let apiRes = await uploadDoc("GST_CERTIFICATE", currentBusinessPartnerId, file)
  ...
  const generateRequest = (fileId: string, gstin: string, businessPartnerId: string): IUploadAndSaveDocument => {
    return {
                        ...
                        <IonCol size="8" className="">
                          <IonItem lines="none" className="input-item">
                            ...
                            >
                              ...
                              {
                                dropDownList.map((item, index) => <IonSelectOption key={index} value={`${item.value}`}>
                                  {item.label}
                                </IonSelectOption>
                                )
                              }
                          ...
                          <IonCol className="" size="auto">
                            <div className="d-flex ion-align-items-center mr-6">
                              ...
                              <div className="status-box-inbox">
                                <span className="fw-700 fs-12 dark">{getUploadedStatusCount().uploaded}</span>/
                                <span className="fs-12 fw-400">{getUploadedStatusCount().total}</span>
                              </div>
                              <div>
                                ...
                                <IonGrid>
                                      ...
                                      <IonCol>
                                        <div className="file-main-wrapper p-relative">
                                          ...
                                          <div className="file-size-wrap ion-text-center">
                                            <p className="fs-14 fw-400 mb-0">{t("FILE_SUPPORTED")}: {getAllowedGstFilesDisplay()}</p>
                                  ...
                                  <IonRow>
                                    <IonCol size="3">
                                      {
                                        uploadObj?.file && (
                                          <FileDisplayComponent onDelete={
                                            ()=>{
                                              setUploadObj({...defaultFileObj})
                                            }
                                          } showDelete={true} uploadedFile={uploadObj.file}></FileDisplayComponent>
                                        )
                                      }
                              ...
                              <div className="details-section">
                                ...
                                <IonGrid className="certificate-wrapper">
                                  <IonRow className="certificate-box ion-align-items-center">
                                    <IonCol size="3">
                                      <FileDisplayComponent displayTitle={false} uploadedFile={activeDocument?.files[0]} showDelete={false}></FileDisplayComponent>
                                    </IonCol>
                                    <IonCol size="7" class="">
                                  ...
                                  <DocumentStatusComponent colorClass={getDisplayFileDetails().colorClass}
                      ...
                      <IonCardContent className="footer-gradient py-0">
                        <IonGrid>
                          <IonRow className="">
                            ...
                            <ActyvButton onClick={submitDocument} text={t("SAVE")} isLoading={submitLoading} disabled={submitDisabled()}></ActyvButton>
                              
                            </IonCol>
                            
                                <IonCol size="6" className="ion-text-center">
                                  <IonButton className="fs-14 fw-500 clear-btn-gray" fill="clear"
